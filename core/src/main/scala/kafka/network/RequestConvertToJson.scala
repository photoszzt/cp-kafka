...
object RequestConvertToJson {
  def request(request: AbstractRequest): JsonNode = {
    request match {
      ...
      case req: GetTelemetrySubscriptionRequest => GetTelemetrySubscriptionRequestDataJsonConverter.write(req.data, request.version)
  ...
  def response(response: AbstractResponse, version: Short): JsonNode = {
    response match {
      ...
      case res: GetTelemetrySubscriptionResponse => GetTelemetrySubscriptionResponseDataJsonConverter.write(res.data, version)
