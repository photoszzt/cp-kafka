...
class ClientInstanceSelectorTest {
  ...
  def testClientMatchingPattern(): Unit = {
    ...
    assertTrue(selector.isMatched(
      Map(CmClientInformation.CLIENT_SOFTWARE_NAME -> "Confluent.*", CmClientInformation.CLIENT_SOFTWARE_VERSION -> "89.2.0")))
    assertTrue(selector.isMatched(
      Map(CmClientInformation.CLIENT_SOFTWARE_NAME -> "Confl.ent.*a", CmClientInformation.CLIENT_SOFTWARE_VERSION -> "^8.*0")))
    assertTrue(selector.isMatched(
      Map(CmClientInformation.CLIENT_SOFTWARE_NAME -> "Confluent.Java$", CmClientInformation.CLIENT_SOFTWARE_VERSION -> "8..2.*")))
    assertFalse(selector.isMatched(
      Map(CmClientInformation.CLIENT_SOFTWARE_NAME -> "Confluent.Java", CmClientInformation.CLIENT_SOFTWARE_VERSION -> "8.1.*")))
    assertFalse(selector.isMatched(
